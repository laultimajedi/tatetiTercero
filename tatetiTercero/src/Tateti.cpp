...
void Tateti::inicializarTateti() {
	...
	Pila<Tablero*>* ultimosTableros = new Pila<Tablero*>();
	...
	Cola<Jugador*> *turnos = new Cola<Jugador*>();
...
void Tateti::colocarFicha (Jugador* actual){
    ...
    int n = this->tablero->getCantidadFichas();
	if(this->posicionValida(x, y, z)){
		this->tablero->setCantidadFichas(n + 1);
...
bool Tateti::movimientoValido(int x1, int y1, int z1, int x2, int y2, int z2, Jugador* actual){
	if(!sonFichasIguales(this->tablero->getCasillero(x1, y1, z1)->getValor(), actual->getFicha()) || !this->posicionValida(x2, y2, z2)){
		return false;
	}
...
void Tateti::moverFicha(Jugador* actual){
	...
	if(this->movimientoValido(x1, y1, z1, x2, y2, z2, actual)){
...
int Tateti::dimensionMayor(int x, int y, int z){
	if( x == y && y == z){
		return x;
	}
	if(x >= y && x >= z){
...
void Tateti::ejecutar(){
	int dim = dimensionMayor(this->tablero->getAnchura(), this->tablero->getAltura(), this->tablero->getProfundidad());
